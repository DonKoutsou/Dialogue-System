...
class SP_MultipleChoiceConfig: ScriptAndConfig
{
	...
	protected ref array<ref DialogueMultiTextConfig> m_MultiChoiceTexts;
	...
	string GetActionText(int TextID)
    {
        ...
        {
            return m_MultiChoiceTexts[TextID].ActionText;
        }
        else 
        {
            return STRING_EMPTY;
	...
	string GetDialogueText(int TextID)
    {
        if (TextID >= 0 && TextID < m_MultiChoiceTexts.Count()) 
        {
            return m_MultiChoiceTexts[TextID].DialogueText;
        }
        else 
        {
            return STRING_EMPTY;
...
class DialogueMultiTextConfig: ScriptAndConfig
{
